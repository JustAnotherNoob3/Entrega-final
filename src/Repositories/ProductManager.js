...
class ProductManager {
    ...
    async deleteProduct(productId) {
        try {
            await daoProducts.delete(productId);
            let product = (await daoProducts.getById(productId));
            if(product.owner != "admin") return;
            email_sender(product.owner, "Uno de tus productos fue eliminado", "Tu producto "+ product.name + " fue eliminado.")
        } catch { throw "No product with ID " + productId; }
    }
